...
from modules.coord_conv import CoordConv
from data.utils2 import ListModule
...
class RecurrentHourglass(nn.Module):
    ...
    def __generate_pre_post_convs(self):
        ...
        # Pre Conv Layers for contracting paths
        # 3x3 filters & 64 outputs of all blocks
        ...
        # Post Conv Layers for expanding paths
    ...
    def forward_encoding(self, x):
        ...
        for i in range(self.n_levels):
            x = F.relu(self.pre_conv_layers[i](x), inplace=False)
            hidden = self.convgru_cell(x, hidden)
            convgru_outputs.append(hidden)  # save each outputs of contracting paths
    ...
    def forward_decoding(self, convgru_outputs):
        ...
        _last_output = convgru_outputs[self.n_levels - 1]  # final output of contracting path

        post_feature_map = F.relu(_last_conv_layer(_last_output), inplace=False)
        for i in range(self.n_levels - 1)[::-1]:
            ...
            post_feature_map = F.relu(post_feature_map, inplace=False)
