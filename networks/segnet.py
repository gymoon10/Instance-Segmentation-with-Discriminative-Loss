...
class SegNet(nn.Module):
    ...
    def __init__(self, input_ch, sem_classes, ins_classes, pretrained=True, use_coordinates=False):
        super(SegNet, self).__init__()

        self.cnn = models.vgg16(pretrained=True)
        self.pretrained = pretrained  # freeze vgg16 parameters or not
        ...
        self.sem_out = nn.Conv2d(64, sem_classes, kernel_size=3, stride=1, padding=1)
        self.ins_out = nn.Conv2d(64, ins_classes, kernel_size=3, stride=1, padding=1)

        self.ins_cls_cnn = InstanceCounter(512, use_coordinates,
                                           usegpu=True)
    ...
    def forward(self, x):
        ...
        x = self.enc52(x)
        x = self.enc53(x)
        # x: (N, 512, h/32, w/32), ind_5: (N, 512, h/32, w/32)
        ...
        sem = self.sem_out(x1)
        ins = self.ins_out(x2)
        ins_cnt = self.ins_cls_cnn(x)

        return sem, ins, ins_cnt

    def init_vgg_weigts(self):
        self.enc11.conv[0].weight.data = self.cnn.features[0].weight.data
        self.enc12.conv[0].weight.data = self.cnn.features[2].weight.data
        self.enc21.conv[0].weight.data = self.cnn.features[5].weight.data
        self.enc22.conv[0].weight.data = self.cnn.features[7].weight.data
        self.enc31.conv[0].weight.data = self.cnn.features[10].weight.data
        self.enc32.conv[0].weight.data = self.cnn.features[12].weight.data
        self.enc33.conv[0].weight.data = self.cnn.features[14].weight.data
        self.enc41.conv[0].weight.data = self.cnn.features[17].weight.data
        self.enc42.conv[0].weight.data = self.cnn.features[19].weight.data
        self.enc43.conv[0].weight.data = self.cnn.features[21].weight.data
        self.enc51.conv[0].weight.data = self.cnn.features[21].weight.data
        self.enc52.conv[0].weight.data = self.cnn.features[24].weight.data
        self.enc53.conv[0].weight.data = self.cnn.features[26].weight.data

        if self.pretrained==True:  # Freeze VGG16 parameters
            self.enc11.conv[0].weight.requires_grad = False
            self.enc12.conv[0].weight.requires_grad = False
            self.enc21.conv[0].weight.requires_grad = False
            self.enc22.conv[0].weight.requires_grad = False
            self.enc31.conv[0].weight.requires_grad = False
            self.enc32.conv[0].weight.requires_grad = False
            self.enc33.conv[0].weight.requires_grad = False
            self.enc41.conv[0].weight.requires_grad = False
            self.enc42.conv[0].weight.requires_grad = False
            self.enc43.conv[0].weight.requires_grad = False
            self.enc51.conv[0].weight.requires_grad = False
            self.enc52.conv[0].weight.requires_grad = False
            self.enc53.conv[0].weight.requires_grad = False
