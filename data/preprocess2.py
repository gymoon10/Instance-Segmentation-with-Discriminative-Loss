...
def resized_crop(img, i, j, h, w, size, interpolation=Image.Resampling.BILINEAR):
...
class RandomResizedCrop(object):
    ...
    def __init__(self, size_height, size_width, interpolation=Image.Resampling.BILINEAR):
...
def rotate(img, angle, resample=Image.Resampling.BILINEAR, expand=True):
...
def rotate_with_random_bg(img, angle, resample=Image.Resampling.BILINEAR, expand=True):
    ...
    if key == 0:
        ...
        bg = Image.new('RGBA', img.size, (int(mean_color[0]), int(mean_color[1]), int(mean_color[2]), 255))
    elif key == 3:
        median_color = np.median(img_np, (0, 1))
        bg = Image.new('RGBA', img.size, (int(median_color[0]), int(median_color[1]), int(median_color[2]), 255))
...
class RandomRotate(object):
    ...
    def __init__(self, interpolation=Image.Resampling.BILINEAR, random_bg=True):
        self.interpolation = interpolation
