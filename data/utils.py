...
from data.preprocess2 import RandomResizedCrop, RandomHorizontalFlip, \
...
class ImageUtilities(object):
    ...
    @staticmethod
    def image_resizer(height, width, interpolation=Image.Resampling.BILINEAR):
        return transforms.Resize((height, width), interpolation=interpolation)

    @staticmethod
    def image_random_cropper_and_resizer(height, width, interpolation=Image.Resampling.BILINEAR):
    ...
    @staticmethod
    def image_random_rotator(interpolation=Image.Resampling.BILINEAR, random_bg=True):
        return RandomRotate(interpolation=interpolation, random_bg=random_bg)

    @staticmethod
    def image_random_90x_rotator(interpolation=Image.Resampling.BILINEAR):
    ...
    @staticmethod
    def image_random_grayscaler(p=0.3):
